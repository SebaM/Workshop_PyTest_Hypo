...
def my_first_strategy():
    return st.one_of(
        st.decimals(min_value=0, allow_nan=False, allow_infinity=False, places=2),
    )


@given(num1=my_first_strategy(), num2=my_first_strategy())
@settings(verbosity=Verbosity.quiet, deadline=datetime.timedelta(seconds=30), max_examples=60)
